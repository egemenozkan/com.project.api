package com.project.enginee.flight.service.impl;
...
import com.project.enginee.flight.service.IFlightTrackingService;
import com.project.flight.enums.AirlineEnum;
...
public class FlightTrackingService implements IFlightTrackingService {
	...
	try {
		...
		for (int i = 0; i < li.size(); i++) {
			...
			case 2:
			    ...
			    break;
			case 3:
			    if (FlightTrackingType.DOMESTIC_ARRIVAL == trackingType || FlightTrackingType.INTERNATIONAL_ARRIVAL == trackingType) {
			...
			case 4:
			    ...
			    if (param.length() > 0) {
				LocalDateTime scheduledTime = flight.getFlightDate().atTime(Integer.parseInt(param.substring(0, 2)),
					Integer.parseInt(param.substring(3)));
				flight.setScheduledTime(scheduledTime);
			    }
			    break;
			case 5:
			    if (param.length() > 0) {
				LocalDateTime estimatedTime = flight.getFlightDate().atTime(Integer.parseInt(param.substring(0, 2)),
					Integer.parseInt(param.substring(3)));
	...
	responseContent = responseContentBuf != null ? responseContentBuf.toString().replace("\"", "").replace("\\", "\"") : null;
	...
	String type = (trackingType == FlightTrackingType.DOMESTIC_ARRIVAL || trackingType == FlightTrackingType.INTERNATIONAL_ARRIVAL) ? "1" : "2";
	String ic = (trackingType == FlightTrackingType.DOMESTIC_ARRIVAL || trackingType == FlightTrackingType.DOMESTIC_DEPARTURE) ? "1" : "2";
	String requestFile = "{\"file\":\"" + requestUrlBuf.toString() + "\",\"type\":" + type + ",\"caption\":\"\",\"ic\":" + ic + ",\"hv\":" + airportId
		+ "}";
	return requestFile;
    }
}
